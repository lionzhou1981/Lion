...
namespace Lion.Encrypt
{
    public class RSA
    {
        ...
        public string DecryptFromByteArray(byte[] _array)
        {
            if (_privateKeyRsaProvider == null)
            {
                throw new Exception("_privateKeyRsaProvider is null");
            }
            return System.Text.Encoding.UTF8.GetString(_privateKeyRsaProvider.Decrypt(_array, RSAEncryptionPadding.Pkcs1));
        }
        #endregion
        ...
        {
            return Convert.ToBase64String(EncryptToByteArray(System.Text.Encoding.UTF8.GetBytes(text)));
        }

        public byte[] EncryptToByteArray(string text)
        {
            return EncryptToByteArray(System.Text.Encoding.UTF8.GetBytes(text));
        }

        public byte[] EncryptToByteArray(byte[] _byteArray)
        {
            if (_publicKeyRsaProvider == null)
            {
                throw new Exception("_publicKeyRsaProvider is null");
            }
            return _publicKeyRsaProvider.Encrypt(_byteArray, RSAEncryptionPadding.Pkcs1);
        }
