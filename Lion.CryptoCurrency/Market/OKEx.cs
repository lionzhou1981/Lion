...
namespace Lion.CryptoCurrency.Market
{
    public class OKEx : MarketBase
    {
        ...
        public OKEx(string _key, string _secret, string _passPhrase = "123456") : base(_key, _secret)
        {
            base.Name = "OKEx";
            base.WebSocket = $"";
            base.HttpUrl = "https://www.okx.com";
        ...
        #region GetTickers
        public Tickers GetTickers(string _type = "SPOT")
        {
            string _url = $"/api/v5/market/tickers?instType={_type}";

            JToken _token = base.HttpCall(HttpCallMethod.Get, "GET", _url, false);
            if (_token == null) { return null; }

            Tickers _tickers = new Tickers();
            JArray _list = _token["data"].Value<JArray>();
            foreach(JToken _item in _list)
            {
                Ticker _ticker = new Ticker();
                _ticker.Pair = _item["instId"].Value<string>();
                _ticker.LastPrice = _item["last"].Value<decimal>();
                _ticker.LastAmount = _item["lastSz"].Value<decimal>();
                _ticker.AskPrice = _item["askPx"].Value<decimal>();
                _ticker.AskAmount = _item["askSz"].Value<decimal>();
                _ticker.BidPrice = _item["bidPx"].Value<decimal>();
                _ticker.BidAmount = _item["bidSz"].Value<decimal>();
                _ticker.Open24H = _item["open24h"].Value<decimal>();
                _ticker.High24H = _item["high24h"].Value<decimal>();
                _ticker.Low24H = _item["low24h"].Value<decimal>();
                _ticker.Volume24H = _item["volCcy24h"].Value<decimal>();
                _ticker.Volume24H2 = _item["vol24h"].Value<decimal>();
                _ticker.DateTime = DateTimePlus.UnixTime2DateTime(_item["ts"].Value<long>());
                _tickers.AddOrUpdate(_ticker.Pair, _ticker, (k, v) => _ticker);
            }
            return _tickers;
        }
        #endregion
